...
def init_header():
    ...
    st.title("CoachPro")
    st.write("Ton coach professionnel.")
...
def user_interface():
    ...
    with col_input:
        user_input = st.chat_input("Veillez saisir votre demande ici...", key="chat_input")
    ...
    if user_input:
        if uploaded_file:
            if uploaded_file.type == "application/pdf":
                ...
                response = st.session_state.chat_agent.ask_vision_model(
    ...
    if st.session_state.generated_text:
        ...
        with colB:
            ...
            st.download_button("Télécharger en Word", word_buffer, "document.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
        
        st.write("Envoyer par mail")
        recipient = st.text_input("Adresse e-mail du destinataire")
        subject = st.text_input("Sujet", "Votre document généré par CoachPro")
        body = st.text_area("Message", "Bonjour,\nVeuillez trouver ci-joint votre document généré par CoachPro.")
        
        colMailPDF, colMailWord = st.columns(2)
        with colMailPDF:
